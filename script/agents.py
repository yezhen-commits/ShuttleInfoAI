...
async def call_mongodb_agent(x:str):
    """call mongodb_agent when you want to find information related to player information like country, birthdate, height and highest ranking"""
    try:
        response = await mongodb_agent.ainvoke({"messages":[HumanMessage(content=f"Search for information related to {x}")]})
        return {"messages": [AIMessage(content=response["messages"][-1].content)]}
    except Exception as e:
